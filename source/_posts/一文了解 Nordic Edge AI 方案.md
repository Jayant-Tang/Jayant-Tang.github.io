...
![](https://jayant-blog-imgs.oss-cn-hangzhou.aliyuncs.com/v2-eb7b063dbbcb2e9be50f5ea11b56ef98_b.webp)

如上图为“全连接神经网络”运行时（**推理时**）的示意图，把一个数字“2”的图像像素展开作为原始数据输入，最终得到一组概率，其中这个数字是 “2” 的概率是最高的。

全连接神经网络的输入就是传感器数据。原始数据可能来自运动传感器、光电传感器、压力传感器、麦克风、ADC 等。

原始数据被视为一组向量，通过一组运算、得到下一层的神经元数值。经过多层计算，最终输出的向量将会给出一组概率值，对应**预设好的**各个分类目标的概率。

如果你在训练时要识别的手势有8种，那么**输出**就是一个长度为 8 的向量，经过归一化处理后，对应这 8 种手势的概率值（ 0 到 1 之间），你可以判定比如概率超过95% 就认为是这个手势。

其中，“每一层神经元”都是一组中间变量，暂时存储运算结果；“每一层运算”都是**线性运算**和**非线性运算**的结合：

- 线性运算就是，每一层的神经元数据向量 \(x\) 都会乘一个矩阵 \(W\)，再加上一个偏置向量 \(b\)；

  ![fully-connected](https://jayant-blog-imgs.oss-cn-hangzhou.aliyuncs.com/undefined651a53a2c8da6e91fdf5114d9b3db40e.png)

- 非线性运算就是，上一步线性运算的结果会套一层非线性的激活函数**σ()**，作用是引入非线性成分。常见的激活函数有：
  ![各种激活函数](https://jayant-blog-imgs.oss-cn-hangzhou.aliyuncs.com/undefined321bcf9c25c941747d3366924b47d6ce.jpeg)



合并起来，每一层神经元向量（***x***）到下一层神经元向量（***y***)之间的公式：
$$
y=\sigma(Wx+b)
$$
 \(W\) ：权重矩阵，代表上一层神经元如何影响这一层神经元。

 \(b\)：偏置向量，代表这一层神经元的固有偏好。前一层即使为0，也会有一个输出。

\(\sigma()\)：激活函数。如果没有激活函数，再多层权重矩阵和偏置向量都只会等价成一次线性计算。有了激活函数，才能让模型拟合现实中复杂的非线性问题。

> 【补充】输入数据的类型
>
> （1）多通道一维数据
>
> 前面按照图片展示，数据都是"一维"的，是因为示意图是把输入作为一个一维的向量。
>
> 实际上，如果是音频、加速度传感器之类时间序列数据，可能有多通道的情况。如音频的左、右声道；加速度、角速度传感器的 X Y Z 三轴等。这种多通道的数据也被认为是**一维**的，因为数据只会沿着时间的方向增长，而通道数是固定的。这种被称为多变量一维时间序列（Multivariate 1D Time-series）。
>
> 对于多变量时间序列，可以在输入层直接合并。以双通道为例：
> $$
> z=W_{1}x_{1}+W_{2}x_{2}+b
> $$
> 在物理学中，不同量纲的物理量（如加速度和角速度）是绝对不能直接相加的。但在深度学习中，全连接层通过对权重矩阵 \(W\) 的学习，巧妙地打破了这一限制。不同量纲的物理信号被统一映射为了**无量纲的、通用的虚拟空间特征值**。此时，加法和偏置 \(b\) 的相加才具有了数学合规性。
>
> （2）二维数据
>
> 如果是图像这类二维数据，直接把整个图像拆成一行一行来拼接成一维向量，丢进全连接神经网络强行计算，也是可以的。但是，这样会丢失图像数据中相邻像素之间的空间几何关系。对神经网络训练产生灾难性的影响。
>
> 因此一般不会用全连接神经网络来处理图像，而是用卷积神经网络（CNN）等其他网络结构。
>
> 后续会介绍 Nordic 的两种方案，对二维图像数据的支持情况不同。
>
> 1. Neuton AI：适合处理一维时间序列数据，多通道的也可以。但不适合处理包含空间信息的二维数据（如图像）。
> 2. Axon NPU：硬件支持卷积层（支持 1D 卷积和 2D 卷积），全连接层和池化层。适合做低分辨率视觉和复杂音频处理。

## 神经网络模型的训练

权重***W***和偏置***b***构成了神经网络模型的核心**参数**，存放在固件的只读数据区。所谓的**机器学习**就是通过大量数据的训练，让神经网络找到最合适的参数。

所谓模型的训练，就是在选择好模型的层数、精度、激活函数等**超参数（Hyperparameters）**之后，从零开始迭代出整个模型中所有的权重矩阵和偏置。

> 一般称工程师选择的参数为“超参数”，用于区分模型自己的参数。

因此，我们需要预先准备好大量的**已经标注的数据（Labeled data）**。数据（Data）和标签（Label）就像作业的题目和答案：模型在训练过程中不断尝试给出预测，并根据与标准答案的偏差来微调内部权重，直到输入数据能高概率地指向正确标签。
...
Neuton 神经网络体积小，功耗低，直接在 CPU 上运行，支持 Nordic 目前所有的 SoC 平台。
