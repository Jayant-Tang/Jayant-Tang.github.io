...
如果你的电脑可以正常访问 GitHub，那么可以直接运行成功。首次运行时，会连接到 `raw.githubusercontent.com` 去获取最新的工具软件列表，然后下载对应操作系统的工具。但这个网站在国内通常无法直接访问，运行会报以下错误：
...
我们先用一台可以正常访问 GitHub 的电脑访问一下日志中提到的无法访问的网址，可以看到这个 JSON 文件的内容：
